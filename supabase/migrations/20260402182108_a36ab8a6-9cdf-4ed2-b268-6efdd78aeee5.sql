...
BEGIN
  ...
  -- Try to find by phone first (normalized)
  ...
  -- Fallback: find by email if provided
  ...
  IF v_customer_id IS NULL THEN
    v_is_new := true;
    -- Use email or generate placeholder from phone
    v_customer_id := public.upsert_restaurant_customer(
      p_restaurant_id,
      CASE WHEN v_sanitized_email != '' THEN v_sanitized_email ELSE v_sanitized_phone || '@phone.local' END,
      p_name,
      p_phone,
      'manual'::text,
      NULL::boolean,
  ...
  SET
    ...
    customer_email = CASE WHEN v_sanitized_email != '' AND customer_email LIKE '%@phone.local' THEN v_sanitized_email ELSE customer_email END,
    vip = CASE
