-- Corrigir o trigger para verificar se o customer_id existe antes de chamar increment_customer_loyalty
...
BEGIN
  IF NEW.status = 'seated' AND (OLD.status IS NULL OR OLD.status != 'seated') THEN
    ...
    IF NEW.user_id IS NOT NULL AND NEW.user_id != '00000000-0000-0000-0000-000000000000' THEN
      -- Verificar se o customer_id existe na tabela customers
      SELECT EXISTS(SELECT 1 FROM public.customers WHERE id = NEW.user_id) INTO v_customer_exists;
      
      -- Só processar se o cliente existir
