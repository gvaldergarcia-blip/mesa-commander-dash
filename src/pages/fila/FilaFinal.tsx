 ...
 * SEM tela intermediária de consentimento - tudo inline na tela final
 * Termos obrigatórios para ver posição, marketing opcional
 */
...
export default function FilaFinal() {
  ...
  // Consentimento inline na tela final
  ...
  // Aceitar termos e salvar consentimento em background
    ...
    void (async () => {
      try {
        let { error } = await supabase.rpc('upsert_restaurant_customer', {
          ...
          p_opt_in_source: marketingOptin ? 'queue_gateway' : null,
        });
        if (error?.message?.includes('p_opt_in_source')) {
          ({ error } = await supabase.rpc('upsert_restaurant_customer', {
            p_restaurant_id: queueInfo.restaurant_id,
            p_email: email,
            p_name: queueInfo.customer_name || null,
            p_phone: queueInfo.customer_phone || null,
        ...
        if (error) console.warn('[FilaFinal] Consentimento não persistido:', error);
      } catch (e) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-background flex items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-xl border-0">
        ...
        <CardContent className="space-y-6 pt-6">
          {/* Se termos NÃO aceitos, mostrar consentimento inline + posição borrada */}
          {!showPosition ? (
            <>
              {/* Posição borrada como teaser */}
              <div className="bg-orange-50 rounded-xl p-6 text-center relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <p className="text-orange-700 font-semibold text-sm bg-white/80 px-4 py-2 rounded-lg shadow">
                    🔒 Aceite os termos para ver sua posição
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 p-4 bg-muted/50 rounded-lg border">
                <Checkbox
                  ...
                  className="mt-0.5"
                />
                <Label htmlFor="terms-fila" className="text-sm font-medium leading-tight cursor-pointer">
                  Li e aceito os{' '}
              ...
              <div className="flex items-start space-x-3 p-4 bg-orange-50 rounded-lg border border-orange-200">
                ...
                <div className="space-y-1">
                  <Label htmlFor="marketing-fila" className="text-sm font-medium leading-tight cursor-pointer">
                    Quero receber ofertas e novidades por SMS.
                  </Label>
                  <p className="text-xs text-muted-foreground">Opcional. Você pode cancelar a qualquer momento.</p>
                </div>
              </div>
