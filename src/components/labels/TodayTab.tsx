    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Button
          size="lg"
          variant="secondary"
          onClick={() => onQuickAction("shopping")}
          className="h-auto py-4 flex-col gap-1.5"
        >
          <ShoppingCart className="h-5 w-5" />
          <span className="text-xs font-semibold">Lista de compras</span>
          {missingProducts.length > 0 && (
            <span className="text-[10px] font-bold text-destructive">
              {missingProducts.length} em falta
            </span>
          )}
        </Button>
        <Button
      ...
      <Card className="p-4 md:p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold uppercase tracking-wider">Diário operacional</h3>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10 text-muted-foreground text-sm">
            Nenhum evento ainda. Comece registrando um recebimento ou emitindo uma etiqueta.
          </div>
        ) : (
          <ol className="relative border-l border-border/60 ml-2">
            {events.map((e) => {
              const meta = EVENT_META[e.event_type] ?? EVENT_META.manipulation;
              const Icon = meta.icon;
              return (
                <li key={e.id} className="pl-6 pb-4 relative last:pb-0">
                  <span
                    className={cn(
                      "absolute -left-[13px] top-0 h-6 w-6 rounded-full border flex items-center justify-center",
                      meta.bg
                    )}
                  >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{describe(e)}</p>
